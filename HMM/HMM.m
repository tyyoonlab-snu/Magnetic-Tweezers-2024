...
cpath = 'E:\F\HMM code'; % position of the HMM file
cd(cpath);
saving = 1;
SNRcorrection = 1;
fps = 1200; %Hz;
...
Precondition2 = str2num(input('Do you know positions of states in the system ? (yes = 1, no = otherwise): ','s'));
...
if Precondition1 == 1 % Known number of state
    ...
    if Precondition2 == 1 % Known position of state
        if Precondition3 == 1
            ...
            for k = 1:numdata
                ...
                data{1} = filtdata((fix(fps/(3*Precondition0)):end));
            ...
            for k = 1:length(dividing)
                ...
                for i = 1:size(HMMtrace_total{k},2)
                    ...
                    plot((1:length(datarawarr{k}{i}))./fps,datarawarr{k}{i},'k','LineWidth',0.25)
                    plot((1:length(datarawarr{k}{i}))./fps,medfilt1(datarawarr{k}{i},round(fps./Precondition)),'y','LineWidth',0.5)
        ...
        if Precondition3 == 1
            ...
            for k = 1:numdata
                ...
                figure(100+k);hold on; box on; grid on;
                plot((1:length(dataraw{k}(fps/(3*filterwindow(k)):end)))./fps,dataraw{k}(fps/(3*filterwindow(k)):end),'k','LineWidth',0.25)
                plot((1:length(dataraw{k}(fps/(3*filterwindow(k)):end)))./fps,medfilt1(dataraw{k}(fps/(3*filterwindow(k)):end),round(fps./Precondition)),'y','LineWidth',0.5)
                plot((1:length(HMMtrace_total{k}{1}))./fps,HMMtrace_total{k}{1},'r','LineWidth',0.5)
                set(gca,'fontsize', 12, 'linewidth', 1, 'fontweight', 'bold','TickLength',[0.02 0.02])
                sc = 40;
                for i =1:Q
    ...
    plot(2:Qmax,BIC(2:Qmax),'k','LineWidth',1)
    ...
    for p = 2:Qmax
        ...
        for i = 1:size(HMMtrace,2)
            ...
            plot((1:length(dataraw{i}))./fps,dataraw{i},'k','LineWidth',0.25)
            plot((1:length(data{i}))./fps,data{i},'y','LineWidth',0.5)
